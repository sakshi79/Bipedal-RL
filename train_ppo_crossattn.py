...
def train_ppo_crossattn(clip_ratio=0.2, gamma=0.99, gae_lambda=0.8, entropy_coef=1e-4, mini_batch_size=64,
              actor_lr=3e-4, critic_lr=4e-4, n_epochs=5, T=2048, n_episodes=1000, hardcore=False, seq_len=4):
    ...
    for episode in (pbar := tqdm(range(n_episodes), desc="avg reward = N/A / max reward = N/A ")):
        ...
        if episode_reward > max_reward:
            ...
            steps_at_maxr = steps
            torch.save(agent.actor.state_dict(), f"models/ppocrossattn_actor_gamma:{gamma:.3f}_lambda:{gae_lambda:.3f}_batch:{mini_batch_size}_clip:{clip_ratio:.3f}.pt")
            torch.save(agent.critic.state_dict(), f"models/ppocrossattn_critic_gamma:{gamma:.3f}_lambda:{gae_lambda:.3f}_batch:{mini_batch_size}_clip:{clip_ratio:.3f}.pt")
